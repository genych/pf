 ...
 */
final class Version20200731131756 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__item AS SELECT id, shipping_id, sku, price, title FROM item');
        ...
        , price INTEGER NOT NULL, title VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_1F1B251E4887F3F8 FOREIGN KEY (shipping_id) REFERENCES shipping_price (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO item (id, shipping_id, sku, price, title) SELECT id, shipping_id, sku, price, title FROM __temp__item');
        ...
        $this->addSql('CREATE INDEX IDX_F5299398BBC3C73C ON "order" (shipping_info_id)');
        ...
        $this->addSql('CREATE INDEX IDX_DE686795126F525E ON package (item_id)');
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        , price INTEGER NOT NULL, title VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO item (id, shipping_id, sku, price, title) SELECT id, shipping_id, sku, price, title FROM __temp__item');
        $this->addSql('DROP TABLE __temp__item');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_1F1B251E4887F3F8 ON item (shipping_id)');
        $this->addSql('DROP INDEX IDX_F5299398BBC3C73C');
        $this->addSql('DROP INDEX IDX_F52993989395C3F3');
        $this->addSql('CREATE TEMPORARY TABLE __temp__order AS SELECT id, shipping_info_id, customer_id, price FROM "order"');
        $this->addSql('DROP TABLE "order"');
        ...
        $this->addSql('CREATE INDEX IDX_2812CEDE8D9F6D38 ON order_package (order_id)');
        $this->addSql('CREATE INDEX IDX_2812CEDEF44CABFF ON order_package (package_id)');
        $this->addSql('DROP INDEX IDX_DE686795126F525E');
        $this->addSql('CREATE TEMPORARY TABLE __temp__package AS SELECT id, item_id, quantity FROM package');
        $this->addSql('DROP TABLE package');
